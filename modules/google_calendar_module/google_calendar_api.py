...
class GoogleCalendarAPI:
  ...
  def get_vacation_list(self, option):
    result = list(filter(lambda e: self.is_vacation(e["summary"]), self.get_event_list(option=option)))
    return result
  ...
  # 일정에서 휴가만 필터링 하는 방식
  # option : 일별, 월별
  def get_common_event_list(self, option):
    result = list(filter(lambda e: not self.is_vacation(e["summary"]), self.get_event_list(option=option)))
    return result
  ...
  def get_event_list(self, option):
    ...
    time_min = datetime(year = now.year, month = now.month, day = now.day).astimezone(SEOUL_TIMEZONE)
  ...
  def make_response(self, event):
    response = {
      ...
      "start" : event["start"].get("dateTime") if event["start"].get("dateTime")  else event["start"].get("date"),
      "end" : event["end"].get("dateTime") if event["end"].get("dateTime") else event["end"].get("date"),
